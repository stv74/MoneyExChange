(function ($) {
	...
	methods.init = function (userOptions) {
		...
		return this.each(function () {
			...
			if (!data) {
				...
				var original = obj,
					placeholder = $('<div').attr('id', obj.attr('id') + '-dd-placeholder');
					...
					ddOptions.append(
						'<li>' +
							...
							(item.imageSrc
								...
								: '') +
							(item.text
								? ' <label class="dd-option-text">' + item.text + '</label>'
								: '') +
							(item.description
	...
	methods.select = function (options) {
		return this.each(function () {
			...
			if (options.id) selectId($(this), options.id);
		});
	};
